...
def main(is_cpu, recorded_path):
    ...
    raw_feed = None
    filter_feed = None
    camera1_thread = threading.Thread(target=live_feed, args=(shutdown_event, recorded_path, camera1_frame_buffer, raw_feed, filter_feed))
    ...
    try:
        while True:
            ...
            if raw_feed is not None:
                cv2.imshow("raw", raw_feed)
            if filter_feed is not None:
                cv2.imshow("filter", filter_feed)
