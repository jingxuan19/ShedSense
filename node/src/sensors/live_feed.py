...
def live_feed(shutdown_event, is_recorded, frame_buffer, K, D):
    ...
    Args:
        shutdown_event (threading.Event): Shutdown flag
        is_recorded (bool): _description_
        frame_buffer (Queue.queue): Shared queue with main node thread
        K (np.array): Camera intrinsic matrix
        D (np.array): Distortion coefficients
    """
    # For testing, is_record contains file path
    if is_recorded:
