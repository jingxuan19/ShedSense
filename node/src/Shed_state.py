...
class Shed_state:
    ...
    def __init__(self):
        ...
        self.logger = logging.getLogger("Live feed")
        handler = logging.FileHandler(f"/home/shedsense1/ShedSense/node/logs/shed_state/{datetime.date.today()}_livefeed")
            
        self.logger.setLevel(logging.INFO)
        self.logger.addHandler(handler)
        
    def history_update(self, measurements):
    ...
    def publish_state(self):
        # self.Node_MQTT_Client.publish("ShedSense/node/frame", self.frame)
        self.Node_MQTT_Client.publish("ShedSense/node/status", self.status)
